...
describe('SaveSurveyResult Controller', () => {
  ...
  test('Should return 403 if LoadSurveyById returns null', async () => {
    ...
    expect(httpResponse).toEqual(forbiddenError(new InvalidParamError('surveyId')))
  })
  ...
  test('Should return 403 if an invalid answer is provided ', async () => {
    ...
    expect(httpResponse).toEqual(forbiddenError(new InvalidParamError('answer')))
  })
  ...
  test('Should return 200 on success', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.handle(makeFakeRequest())
    expect(httpResponse).toEqual(successRequest(mockSurveyResultModel()))
  })
})
